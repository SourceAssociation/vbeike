...
class PublicAction extends Action{
	...
	public function doLogin() {                 //本地登录  
		...
		$password =	$_POST['password'];    //未加密

		if(!$password){
			$this->error(L('please_input_password'));
		}
		
		$result = service('Passport')->loginLocal($username,$password,intval($_POST['remember']));   //未加密的密码
	...
	public function refreshToken(){				//刷新人人accessToken代码
		...
		$rruser=M('rruser')->where('uid='.$this->mid)->find();
		$authApi=new RenrenOAuthApiService;
		if($rruser){
			if(intval( time() ) > intval( $rruser['expires_in'] ) + intval( $rruser['gettime'] )){   //过期刷新
				$post_params=array(
					'client_id'=>$config->APIKey,
					'client_secret'=>$config->SecretKey,
					'refresh_token'=>$rruser['refresh_token'],
					'grant_type'=>'refresh_token',
				);
				$token_url="http://graph.renren.com/oauth/token";
				$json=$authApi->rr_post_curl($token_url,$post_params);
				if(!$json['error']){
					//更新	accesstoken
					$update=array(
					...
					M('rruser')->where('uid='.$this->mid)->save($update);
					$_SESSION['access_token']=$json['access_token'];    //人人token
				}
			}else{      //没有过期 继续使用
				$_SESSION['access_token']=$rruser['access_token'];	   //人人token
			}	
		}
	}
	
	//	model('Message')->postMessage($message_data,  M('user')->getField('uid','admin_level=1')); 
	public function rr_login(){     		//	人人登陆  
		Session::start();
			$code=$_GET['code'];
			if(isset($code)&&!empty($code)){
					...
					if(!$access_info['error']){
						$user=M('rruser')->where('rr_uid='.$access_info['user']['id'])->find();
						if($user){
							//更新人人信息		//存入rruser 表中
							$update_rruser=array(
								'expires_in'=>$access_info['expires_in'],
								'access_token'=>$access_info['access_token'],
								'refresh_token'=>$access_info['access_token'],
								'gettime'=>time()
							);
							M('rruser')->where('uid='.$user['uid'])->data($update_rruser)->save();  // update
							$_SESSION['access_token']=$access_info['access_token'];
							$ts_user = D('User', 'home')->getUserByIdentifier($user['uid'],'uid');
							service('Passport')->registerLogin($ts_user,false,null);   //没有password  只能记录一个小时~				
							redirect(U('home/User/index'));	
						}else{
							$this->assign("user",$access_info['user']);
							$_SESSION['access_info']=$access_info;
							$this->display();   		//绑定邮箱
						}
					}else{
...
public function rr_sync(){  	 //	判断是否同步人人 && 站内绑定人人账号
	Session::start();
	include_once(dirname(dirname(__FILE__)).'/class/rr_config.inc.php');
	include_once(dirname(dirname(__FILE__)).'/class/RenrenOAuthApiService.class.php');
	include_once(dirname(dirname(__FILE__)).'/class/RenrenRestApiService.class.php');
	$is_sync=intval($_GET['is_sync']);
	$model=M('rruser');
	$rruser=$model->where('uid='.$this->mid)->find();
	if($is_sync==1){
		if($rruser){
			$model->where('uid='.$this->mid)->save(array('is_sync_rr'=>1));
			$_SESSION['access_token']=$rruser['access_token'];
			echo 1;
		}else{
			$redirecturi=U('home/Public/rr_sync');
			$redirecturi=urlencode($redirecturi);
			$url="http://graph.renren.com/oauth/authorize?client_id=$config->APPID&response_type=code&redirect_uri=$redirecturi&scope=$config->scope&display=page";
			echo '<dl class="pop_sync"><dt></dt>您还未绑定人人帐号, 请点这里<dd><a class="btn_b" href="' . $url . '">开始绑定</a></dd></dl>';
		}
	}else{
		if($rruser){
			$model->where('uid='.$this->mid)->save(array('is_sync_rr'=>0));
			echo 2;
		}
	}
	if($_SESSION['from_url']==1){
		$jumpUrl=U('home/Account/bind');
		unset($_SESSION['from_url']);
	}else{
		$jumpUrl=U('home/User/index');
	}
	...
	if(!empty($code)){
		...
		$post_params=array('client_id'=>$config->APIKey,
			'client_secret'=>$config->SecretKey,
			'redirect_uri'=>U('home/Public/rr_sync'),
			'grant_type'=>'authorization_code',
			'code'=>$code
			);
		$token_url="http://graph.renren.com/oauth/token";
...
public function unbind_rr(){		//解除人人绑定
	$id=M('rruser')->where('uid='.$this->mid)->delete();
	if(id){
		echo 1;
	}
}
	
 public	 function bind_email(){  	  //人人登陆绑定邮箱  
		//服务器端验证代码
		$email=t( $_POST['email'] );
		$pwd1=t( $_POST['pwd1'] );
		$pwd2=t( $_POST['pwd2'] );	
		...
		if(!isValidEmail($email)){
			$this->error("邮箱格式不正确奥，亲~");
		}
		if(!isEmailAvailable($email)){
			$this->error("该邮箱也被占用了，请换一个试试吧~");
		}
		...
		if($pwd1!=$pwd2){
			$this->error("两次输入的密码不一样奥~");
		}
		
		$access_info=$_SESSION['access_info'];		
		unset($_SESSION['access_info']);
		
		$rr_user=$access_info['user'];		
		$data['email']     = $email;
		$data['password']  = md5($pwd2);
		$data['uname']	   = $rr_user['name'];
		$data['ctime']	   = time();
		$data['is_active']=1;   //激活
		...
		//头像 嘿嘿
		D('Avatar')->saveAvatar($uid,$rr_user['avatar'][3]['url']);	
				
		//存入rruser 表中
		$rruser=array(
			'rr_uid'=>$rr_user['id'],
			'uid'=>$uid,
			'rr_name'=>$rr_user['name'],
			'is_sync_rr'=>0,   	//默认不同步人人
			'expires_in'=>$access_info['expires_in'],
			'access_token'=>$access_info['access_token'],
			'refresh_token'=>$access_info['access_token'],
			'gettime'=>time()
		...
		// 置为已登录, 供完善个人资料时使用		
		service('Passport')->loginLocal($email,$pwd2,true);    //false
		
		include_once(dirname(dirname(__FILE__)).'/class/RenrenRestApiService.class.php');
		$authApi=new RenrenRestApiService;
		$param = array('uids'=>$rr_user['id'],'fields'=>'sex','access_token'=>$access_info['access_token']);
		$res = $authApi->rr_post_curl('users.getInfo', $param);//curl函数发送请求

		$info['sex']   	  = intval($res[0]['sex']);
		$info['province'] = 1;
		$info['city']     = 10;
		$info['location'] = getLocation(1, 10);   //默认北京市海淀区
		$info['is_init']  = 1;   	//初始化
		M('user')->where("uid={$uid}")->data($info)->save();
		// 关联操作
		$this->registerRelation($uid);
		S('S_userInfo_'.$uid,null);
		$jumpUrl=U('home/User/index');
		$this->success("登陆成功，页面跳转中...",$jumpUrl);
	} 
	...
	public function doRegister()
	{
		...
		if ($need_email_activate == 1) { // 邮件激活
			...
			// 置为已登录, 供完善个人资料时使用
			service('Passport')->loginLocal($uid);       //return false  输出：NULL
			//注册完毕，跳转至帐号修改页
	...
	public function userinfo()
	{
		...
		if ($_POST) {
			...
			redirect(U('home/Public/tag_verify'));    //	实名认证以及学院标签添加
	...
	public function followuser()
	{
		if ($_POST) {
			...
			//tag 	或者说是学院相同
			$tag_model=d('usertag');
			$user_tag=$tag_model->getusertaglist($this->mid);
			$same_tag_user=$tag_model->dosearchtag($user_tag[0]['tag_name']);
			array_slice($same_tag_user,0,8);   //每次8个
 			$i=0;
			foreach($same_tag_user['data'] as $tag_user){
					$uid=$tag_user['uid'];
					if($uid==$this->mid){
						continue;
					}
				 	$data[$i]['uid']=$tag_user['uid'];
					$data[$i]['count']=$tag_user['user']['follower'];
					$data[$i]['uname']=$tag_user['user']['uname'];
					$data[$i]['location']=$user_tag[0]['tag_name']; 
					$i++;
			}
			unset($i);
			$this->assign("same_tag_users",$data); 
