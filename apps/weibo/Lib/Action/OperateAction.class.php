...
class OperateAction extends Action{
    ...
    function publish(){
		...
		//同步人人
		if($_POST['rr_sync']=="renren"){
			$this->sync_rr_feeds($_POST);
		}
	...
	//	同步人人新鲜事
	public function sync_rr_feeds($data){
		Session::start();
		$access_token=$_SESSION['access_token'];
		$rrObj = new RenrenRestApiService;
		
		if($data['publish_type']=="3"){
			$feeds=array(
				'name'=>"视频分享",
				'description'=>$data['content'],
				'url'=>$data['publish_type_data'],
				'access_token'=>$access_token
			);
			$post_id=$rrObj->rr_post_curl('feed.publishFeed', $feeds);//curl函数发送请求	
		}elseif($data['publish_type']=="0"){
			$feeds=array(
				'name'=>"微北科",
				'description'=>$data['content'],
				'url'=>"www.wbeike.com",
			...
			$post_id=$rrObj->rr_post_curl('feed.publishFeed', $feeds);//curl函数发送请求	
			$_SESSION['blog']=$post_id;
		}elseif($data['publish_type']=="8"){	//博客
			$end=strripos($data['content'],'】');		//找到】出现的位置
			$title=substr($data['content'],1,$end-1);   //日志标题
			$blog=array(
				'title'=>$title,
				'content'=>$data['content'],
				'access_token'=>$access_token
			);
			$_SESSION['blog']=$rrObj->rr_post_curl('blog.addBlog',$blog);
		}	
		Session::pause();
	}
