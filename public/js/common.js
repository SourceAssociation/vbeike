...
ui = window.ui ||{
	...
	emotions:function(o){
		...
		this.html = '<div class="talkPop alL" id="emotions">'
				 ...
				 + '<div class="close" style="height:30px;line-height:30px;background-color:#F8FAFC;padding:0 10px;position:relative;*width:420px"><a onclick=" $(\'#emotions\').remove()" class="del" href="javascript:void(0)" title="关闭"> </a><span>常用表情</span></div><div id="showEmotion" style="display:none;" class="showEmotion"></div>'
		...
		this.showContent = function(data){  //显示表情内容
			...
			$.each(data,function(i,n){
				content+='<a href="javascript:void(0)" title="'+n.title+'" onclick="ui.emotions_c(\''+n.emotion+'\',\''+target_set+'\')"><img class = "emotionsImg" src="'+ _THEME_ +'/images/expression/'+n.type+'/'+ n.filename +'" width="24" height="24" /></a>';
			});
			content+='<div class="c"></div>';
			$('#emot_content').html(content);

                                 	$('.emotionsImg').hover(function(){
				$('#showEmotion').css('display','');
				$('#showEmotion').html('<img  src="'+$(this).attr('src')+'"  />');
			},
			function(){
				$('#showEmotion').html('');
				$('#showEmotion').css('display','none');
			});	
		};
