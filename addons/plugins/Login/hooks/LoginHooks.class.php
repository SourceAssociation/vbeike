...
class LoginHooks extends Hooks {
    ...
    public function login_input_footer($param) {    		// {:Addons::hook('login_input_footer',array('regInfo'=>$regInfo))}
        Session::start();
		include_once(dirname(dirname(__FILE__)).'/lib/rr_config.inc.php');  	 //   xiangduilujing
		$regInfo = $param['regInfo'];
		$platform_options = model('AddonData')->lget ( 'login' );
		$data = self::$validLogin;
		$platform = array ();
        ...
        if ($regInfo ['register_type'] == 'open' && ! empty ( $platform )) {
			...
			//$image=$this->htmlPath."/html/image/renren.png";
			//$redirecturi=urlencode($config->redirecturi);
