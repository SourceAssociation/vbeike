...
class PassportService extends Service
{
	 ...
	 */
	public function getLocalUser($identifier, $password = null)     //未加密   未加密的密码获
	{
		if (empty($identifier))
			return false;
		...
		//echo $identifier_type;    	 // 可以用用户名获取user  而不是  email
	 ...
	 * @return boolean
	 */
	public function loginLocal($identifier, $password = null, $is_remember_me = false)   //未加密
	{
		$user = $this->getLocalUser($identifier, $password);
		return $user ? $this->registerLogin($user, $is_remember_me,$password) : (UC_SYNC?$this->ucLogin($identifier,$password):false);   //未加密   未加密的密码获
	}

	/**
	 * uc登录或者注册。返回数组    //未加密   未加密的密码获
	 * $result['user'] 用户信息。用于ts系统使用
	 * $result['login'] 同步登录是否成功
	 * @param unknown_type $username
	 * @param unknown_type $password
	 */
	public function ucLogin($username,$password) {     //未加密   未加密的密码获	
		if(isValidEmail($username)){     //如果是email登陆
			...
			if(UC_SYNC){
				...
				if($uc_user[0]>0){     //登陆成功   uid
					...
					if(!$uc_user_ref){
						...
						/* print_r($uc_user);
						echo "<br>";
						echo  $this->isValidEmail($uc_user['3'])."<br>";
						dump($this->isEmailAvailable($uc_user['3']))."<br>";
						exit;  */
						...
						if($this->isValidEmail($uc_user['3']) && $this->isEmailAvailable($uc_user['3'])){
							...
							$message_data['content']="您在ibeike上的邮箱已经绑定到wbeike站点，您可以到".U('home/Account/bind')."查看邮箱绑定。";
						...
						$user['password']  = md5($uc_user['2']);   //md5("littlebear")
						$user['ctime']	   = time();
						$user['is_active'] = 1;
						$user['uid'] = M('user')->add($user);   //添加用户的地方  thinkphp的添加方式
						if ($user['uid']){
							$reg_from_ucenter = 1;
	 ...
	 * @param boolean $is_remeber_me
	 $this->registerLogin($user, $is_remember_me,$password) 
	 */
